...
- [x] **`PRDashboardScreen`** (`screens/ProfileTab/PRDashboardScreen.tsx`, built 2026-08-12; visually revamped 2026-08-13 — "trophy case" direction)
  ...
  - [x] **Real axes + `isAnimated` on both charts** (built 2026-08-19) — `PRProgressionScreen`'s chart and the Dashboard's pinned-card sparklines were axis-less "shape only" sparklines; both now show a recessive x-axis (compact `M/D` labels via new `fmtChartDate` in `utils/prFormat.ts`, one per data point) and y-axis (value ticks, `roundToDigits={0}`, dashed rules), plus `isAnimated` on mount. Progression's chart also dropped `disableScroll` — a long history now scrolls horizontally instead of squeezing labels until they collide, matching `StrengthScoreScreen`'s established chart convention (`yAxisLabelWidth`, `xAxisTextNumberOfLines`, muted `colors.border`/`colors.textSecondary` axis styling)
