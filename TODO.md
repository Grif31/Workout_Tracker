...
- [x] **`PRDashboardScreen`** (`screens/ProfileTab/PRDashboardScreen.tsx`, built 2026-08-12; visually revamped 2026-08-13 — "trophy case" direction)
  ...
  - [x] **Pinned Progression moved above Time Since Last PR; both PR-type pickers switched from pill chips to a segmented control** (built 2026-08-22) — fixed section order is now Hero → Workout Records → Pinned Progression → Time Since Last PR → feed. The independent All/Weight/Reps/Time/Distance pickers (stalled section + feed) are now one connected 5-segment bar (`renderFilterSegmented` in `PRDashboardScreen.tsx`, shared by both) instead of two rows of separate pill buttons — same state/behavior, just a single bordered bar with hairline dividers between segments and the active one accent-tinted
