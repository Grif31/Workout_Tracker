...
def create_template(client, name='Bench Press', muscle_group='Chest'):
    res = client.post('/api/exercises', json={'name': name, 'muscle_group': muscle_group})
    return res.get_json().get('id') or \
        next(e['id'] for e in client.get('/api/exercises').get_json() if e['name'] == name)
...
class TestGetPrsForExercise:
    ...
    def test_does_not_return_records_for_other_exercise(self, client, auth_token):
        t1 = create_template(client, name='Bench Press', muscle_group='Chest')
        t2 = create_template(client, name='Squat', muscle_group='Quads')
